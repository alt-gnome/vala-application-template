...
find ./data/ -iname "*.metainfo.xml.in" | xargs xgettext --no-wrap --package-name="<<APP-NAME>>" --package-version="$version" --from-code=UTF-8 --output=./po/"<<APP-COMMAND>>"-metainfo.pot

msgcat --sort-by-file --use-first --output-file=./po/"<<APP-COMMAND>>".pot ./po/"<<APP-COMMAND>>"-src.pot ./po/"<<APP-COMMAND>>"-blueprint.pot ./po/"<<APP-COMMAND>>"-desktop.pot ./po/"<<APP-COMMAND>>"-metainfo.pot
